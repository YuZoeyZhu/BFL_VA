...
execute_balance_partial <- function(test_site, sites, posterior_phi_full, sim_data_filtered_list) {
  
  library(rstan)
  library(caret)
  
  stan_model <- stan_model(file = "../code/BFL_partial_labeled_balanced.stan") 
  # cat("Processing BFL-partial model for test site:", test_site, "\n")
  ...
  true_test_labels <- as.numeric(sim_data_filtered_list[[test_site]]$mapping_new_to_origin[
    sim_data_filtered_list[[test_site]]$filtered_data$data.truth$Y.t[missing_indx]
  ])
